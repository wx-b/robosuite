...
class MujocoEnv(object, metaclass=EnvMeta):
    ...
    def _post_action(self, action):
        ...
        self.done = (self._check_terminated() or self.t >= self.horizon) and (
            not self.ignore_done
        )
    ...
    def find_contacts(self, geoms_1, geoms_2):
        for contact in self.sim.data.contact[0 : self.sim.data.ncon]:
            if (
                self.sim.model.geom_id2name(contact.geom1) in geoms_1
                and self.sim.model.geom_id2name(contact.geom2) in geoms_2
            ) or (
